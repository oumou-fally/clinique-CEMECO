            ...
            {discussionSelectionnee ? (
              <>
                      ...
                      <div key={msg.id || idx} className={`flex ${isMedecin ? 'justify-end' : 'justify-start'} animate-in fade-in slide-in-from-bottom-2 duration-300`}>
                        <div className={`max-w-[75%] rounded-[2rem] px-6 py-4 shadow-sm relative ${
                          ...
                          {msg.type === 'vocal' && msg.fichier_url && (
                            <div className={`mb-3 p-3 rounded-2xl flex items-center gap-3 ${isMedecin ? 'bg-indigo-500' : 'bg-indigo-50'}`}>
                              <div className={`w-10 h-10 rounded-full flex items-center justify-center ${isMedecin ? 'bg-white text-indigo-600' : 'bg-indigo-600 text-white'}`}>
                            ...
                            <div className={`mb-3 p-4 rounded-2xl flex items-center gap-4 border ${isMedecin ? 'bg-indigo-700/50 border-indigo-400' : 'bg-gray-50 border-gray-200'}`}>
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <a 
                                  ...
                                  className={`text-[10px] font-black uppercase tracking-widest flex items-center gap-1 mt-1 ${isMedecin ? 'text-indigo-200 hover:text-white' : 'text-indigo-600 hover:text-indigo-700'}`}
                                >
                ...
                <div className="p-6 bg-white border-t border-gray-50">
                  ...
                  <form onSubmit={envoyerReponse} className="flex items-end gap-3 bg-gray-50 p-2 rounded-[2rem] border border-gray-100 focus-within:border-indigo-300 focus-within:bg-white transition-all">
                    <div className="flex items-center gap-1 pl-2 mb-2">
                      <button type="button" onClick={() => imageInputRef.current.click()} className="p-2.5 text-gray-400 hover:text-indigo-600 hover:bg-white rounded-full transition">
                        <ImageIcon className="w-5 h-5" />
                      </button>
                      <button type="button" onClick={() => fileInputRef.current.click()} className="p-2.5 text-gray-400 hover:text-indigo-600 hover:bg-white rounded-full transition">
                        <Paperclip className="w-5 h-5" />
                      </button>
                      <button 
                        type="button" 
                        onClick={isRecording ? stopRecording : startRecording} 
                        className={`p-2.5 rounded-full transition relative ${isRecording ? 'bg-red-500 text-white' : 'text-gray-400 hover:text-indigo-600 hover:bg-white'}`}
                      >
                        {isRecording && (
                    ...
                    <div className="flex-1 flex flex-col">
                      {isRecording && (
                        <div className="px-3 py-1 flex items-center gap-2 text-red-500 text-[10px] font-black uppercase tracking-widest animate-pulse">
                      ...
                      <textarea
                        ...
                        placeholder={isRecording ? "Arrêtez l'enregistrement pour envoyer..." : "Tapez votre message..."}
                        disabled={isRecording}
                        className="w-full bg-transparent border-none focus:ring-0 text-sm font-medium p-3 resize-none max-h-32"
                        onKeyDown={(e) => {
                    ...
                    <button
                      ...
                      className="w-12 h-12 flex items-center justify-center bg-indigo-600 hover:bg-indigo-700 disabled:bg-gray-200 text-white rounded-2xl shadow-lg transition-all shrink-0 active:scale-95"
                    >
